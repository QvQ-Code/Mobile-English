<?php 
	...
	if (isset($_GET["del_id"])) {
		...
		if ($word_id > 0) {
			...
			$sql = "SELECT * FROM site_lesson WHERE id=".$word_id;
			$check = $db->query($sql);
			$rows = mysqli_num_rows($check);

			if ($rows > 0) {
			$sql = "DELETE FROM `site_lesson` WHERE `site_lesson`.`id` = $word_id";
    		$a = $db->query($sql);
    		if ($a) {
...
<div class="main-wrapper">
    ...
    <tr>
        <td><?php echo $a++;?></td>
        <td><?php echo $wod["title"];?></td>
        <td><?php echo $wod["status"];?></td>
        <td><?php echo $wod["lesson_range"];?></td>
        <td class="table-action">
            <span style="margin-right: 30px;">
            	<abbr title="Edit">
            	<a href="lesson_edit.php?edit_id=<?php echo $wod["id"]; ?>">
